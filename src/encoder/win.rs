...
impl VideoEncoder {
                    ...
                    let frame = match frame_receiver.recv() {
                        ...
                        match frame {
                            Some((source, timespan)) => {
                                ...
                                println!("sample ts: {}", sample.DecodeTimestamp().unwrap().Duration);
                            }
                            None => {
                                sample_requested.Request()?.SetSample(None)?;
                            }
    ...
    /// if an error occurs.
    pub fn send_frame(&mut self, (surface, ts) : (SendDirectX<IDirect3DSurface>, Instant)) -> Result<(), VideoEncoderError> {
